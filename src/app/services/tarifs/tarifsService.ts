...
export async function createTarif(payload: { categorie: string; prix: number }): Promise<TarifCategorie> {
  const response = await laravelClient.post("/v1/tarifs", payload);
  return mapTarifCategorie(response.data.data || response.data);
}

export async function deleteTarif(id: string): Promise<void> {
  await laravelClient.delete(`/v1/tarifs/${id}`);
}
